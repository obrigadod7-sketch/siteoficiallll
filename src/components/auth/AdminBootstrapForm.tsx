...
export function AdminBootstrapForm({ title = "Configurar Admin" }: Props) {
  ...
  return (
    <section className="mx-auto w-full max-w-[560px] px-6 py-12">
      <h1 className="font-display text-3xl uppercase tracking-[0.14em]">{title}</h1>
      <p className="mt-3 text-sm text-muted-foreground">
        Cria o <strong>primeiro</strong> usuário administrador do Dashboard Kids. Depois que existir um admin, esta
        página bloqueia automaticamente por segurança.
      </p>
      ...
      <Card className="mt-8 p-6 shadow-elev ring-1 ring-border">
            ...
            try {
              ...
              if (error) {
                ...
                toast({
                  ...
                  description: "Não foi possível criar o admin agora. Tente novamente ou use a recuperação de senha.",
                  variant: "destructive",
                });
                return;
              }
        ...
        >
          <div className="grid gap-2">
            <Label htmlFor="bootstrap-email">E-mail administrativo</Label>
            <Input id="bootstrap-email" type="email" autoComplete="email" {...form.register("email")} />
          ...
          {showRecoveryEmail ? (
            <div className="grid gap-2 rounded-md border border-border bg-card p-4">
              <p className="text-sm">Já existe um admin. Clique abaixo para receber um link de recuperação e definir sua senha.</p>
              <Button
                type="button"
                variant="secondary"
                onClick={async () => {
          ...
          {recoveryLink ? (
            <div className="grid gap-2 rounded-md border border-border bg-card p-4">
              <p className="text-sm">1) Abra o link abaixo para definir sua senha. 2) Em seguida, faça login e acesse o dashboard.</p>
              <a
